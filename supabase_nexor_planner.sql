...
create table if not exists public.nexor_planner_clients (
  ...
  contract_type text not null default 'recorrente' check (contract_type in ('recorrente','unico')),
  billing_frequency text not null default 'mensal' check (billing_frequency in ('unico','semanal','quinzenal','mensal','trimestral','semestral','anual')),
  start_date date,
  contract_duration text default '12',
